...
GTEST("Basic Insertion") {
   ...
   cpp_cache::mru_cache<int, int> cache(max_size);
...
GTEST("Basic Removal") {
   ...
   cpp_cache::mru_cache<int, int> cache;
...
GTEST("MRU Semantics") {
   ...
   SHOULD("Evict the Most Recently Accessed") {
      int value{0};
      for (auto i = test_key; max_size > i; ++i) {
         cache.add(i, test_value + i);
         EXPECT_EQ(0, cache.get(i, value));
...
GTEST("Customization") {
   ...
   cpp_cache::mru_cache<int, std::string> cache(max_size);
